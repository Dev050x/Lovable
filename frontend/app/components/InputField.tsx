...
export default function InputField({ onButtonClick, input, setInput, isLoading }: { onButtonClick: () => void, input: string, setInput: React.Dispatch<React.SetStateAction<string>>, isLoading: boolean }) {
    ...
    return (
        <div className="flex flex-col bg-[#121212] border border-[#2E2E2E] rounded-[10px] mx-auto w-full max-w-2xl overflow-hidden shadow-2xl">
            <textarea
                rows={1}
            ...
            <div className="flex items-center justify-between px-3 pb-3 pt-1">
                ...
                {isSignedIn ? (
                    <button
                        className="flex items-center justify-center w-8 h-8 rounded-xl bg-white text-black hover:bg-gray-200 active:scale-95 transition-all font-bold text-base disabled:opacity-80 cursor-pointer disabled:cursor-not-allowed"
                        onClick={() => onButtonClick()}
                        disabled={isLoading}
                    >
                        {isLoading ? <Loader2 className="w-4 h-4 animate-spin text-black" /> : <ArrowUp size={18} />}
                    </button>
                ) : (
